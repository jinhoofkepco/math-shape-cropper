...
- 크롭 미리보기 클릭: 해당 크롭 삭제
- 저장 버튼 왼쪽: 저장 폴더명 입력
- 저장 시: `공통이름_페이지중간이름_개별이름.png` 형식으로 전체 저장
...
OpenCV 없이도 실행되도록 Pillow와 NumPy만 사용합니다. 저장 범위는 마우스로 드래그한 영역을 기준으로 맞추고, 저장 전 흑백 대비와 선명도를 보정합니다. 종이 여백으로 판단되는 밝은 배경은 PNG alpha 채널로 투명화합니다.
